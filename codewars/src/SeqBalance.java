...
public class SeqBalance {
	...
	private static int isGroupMatched(int mididx, char c, char d, String s,
			int[] markIdx) {
		...
		else 
		{
			...
			/*if (s.charAt(mididx+1) == '(')
				isGroupMatched(mididx+1, '(', ')', s, markIdx);
			else if (s.charAt(mididx+1) == '{')
				isGroupMatched(mididx+1, '{', '}', s, markIdx);
			else if (s.charAt(mididx+1) == '[')
				isGroupMatched(mididx+1, '[', ']', s, markIdx);*/
			...
			while (true) 
			{
 				...
 				if (markIdx[closeidx] == 0 && (closeidx-mididx)%2!=0) {
 					markIdx[closeidx] = 2;
 					break;
 				}
 				openidx = closeidx;
